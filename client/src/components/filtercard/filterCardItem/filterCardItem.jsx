...
export const FilterCardItem = ({item: {title, value, color = 'blue'}, filterChange, name}) => {
  return (
    <li className="advertisement-status" key={title}>
    <label className="advertisement-status__label" htmlFor={`filter-${title}`}>
      <input type="checkbox" name={name} value={value} id={`filter-${title}`} className="checkbox-filter advertisement-status__filter" onChange={filterChange}/>
