...
userSchema.virtual('cityName').get(function name() {
  let cityName = cities.find(city => city.value == this.city);
  return cityName;
});

userSchema.virtual('availability').get(function name() {
...
userSchema.methods.passwordSet = function (passwordd) {
  ...
  return this.updateOne({ password, confirmed_email: true }, { new: true }).then(data => {
    if (data && data._id)
      return { ok: 'ok' };
    else
      return { err: 'err' };
